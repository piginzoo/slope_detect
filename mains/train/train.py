...
def initDevParam():
    ROOT_DIR = "/Users/admin/local/creditease/ai/slope_detect/"
    FLAGS.debug = True
    FLAGS.max_steps = 2
    FLAGS.decay_steps = 1
    FLAGS.evaluate_steps = 1
    FLAGS.decay_rate = 0.1
    FLAGS.moving_average_decay = 0.997
    FLAGS.train_dir = ROOT_DIR + "data/train"
    FLAGS.train_label = ROOT_DIR + "data/train.txt"
    FLAGS.train_batch = 1
    FLAGS.validate_dir = ROOT_DIR + "data/validate"
    FLAGS.validate_label = ROOT_DIR + "data/validate.txt"
    FLAGS.validate_batch = 1
    FLAGS.validate_times = 2
    FLAGS.early_stop = 5
    FLAGS.num_readers = 1
    FLAGS.gpu = "0"
    FLAGS.model = "model"
    FLAGS.lambda1 = 1000
    FLAGS.logs_path = ROOT_DIR + "logs"
    FLAGS.pretrained_model_path = ROOT_DIR + "data/vgg_16.ckpt"
    FLAGS.restore = False
    FLAGS.save_checkpoint_steps = 2000
...
def main(argv=None):
    ...
    with tf.Session(config=config) as sess:
        ...
        data_generator = data_provider.get_batch(num_workers=FLAGS.num_readers, label_file=FLAGS.train_label,
        ...
        for step in range(FLAGS.max_steps):
            ...
            if step != 0 and step % FLAGS.decay_steps == 0:
                logger.info("学习率(learning rate)衰减：%f=>%f", learning_rate.eval(),
