...
from main.validate import validate_pb
...
def main(argv=None):
    ...
    with tf.Session(config=config) as sess:
        if FLAGS.restore:
            ...
            builder = tf.saved_model.builder.SavedModelBuilder(FLAGS.model)
        ...
        for step in range(FLAGS.max_steps):
            ...
            if step != 0 and step % FLAGS.evaluate_steps == 0:
                ...
                accuracy_value, precision_value, recall_value, f1_value = validate_pb()

                if accuracy_value > best_accuracy:
                    logger.info("新accuracy值[%f]大于过去最好的accuracy值[%f]，早停计数器重置",accuracy_value,best_accuracy)
                    best_accuracy = accuracy_value
                    early_stop_counter = 0
                    save_model(builder, sess, best_accuracy, step, train_start_time)
                    ...
                    if early_stop_counter % 20:
                        ...
                        save_model(builder, sess, accuracy_value, step, train_start_time)
